...
from app.metrics.metrics import REQUEST_COUNT, CACHE_HITS, CACHE_MISSES, REQUEST_DURATION
...
class RecommendationService:
    ...
    def get_product_recommendations(self, product_id: int, limit: int = 20) -> RecommendationResponse:
        ...
        try: 
            ...
            response = RecommendationResponse(product_id=product_id, 
                                              recommendation_count=len(recommendation_objects), 
                                              recommendations=recommendation_objects)

            self.cache.set(key=cache_key,value=json.dumps(response.model_dump()),ttl=settings.cache_ttl)
